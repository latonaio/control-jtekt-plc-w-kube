...
class JtektPlcData:
    ...
    def is_success(self):
        return True if self.status and self.status.get("status") == "00" else False
...
class JtektPlcDataMulti:
    ...
    @staticmethod
    def create_request(data, decoder_class):
        header_list = {}
        try:
            command = data.get("command")
            array_no = data.get("arrayNo")
            body_bytes = decoder_class.generate_binary(data.get("data"))
            header = SendPacket()
            header.command = bytes.fromhex(command)
            header.block_num = len(body_bytes)+1
            header.array_no = bytes.fromhex(array_no)[::-1]
            header.set_body(body_bytes)
            header_list[array_no] = header
        except Exception as e:
            lprint("cant convert to hex: " + str(e))
            lprint(traceback.format_exc())
            return []
